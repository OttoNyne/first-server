...
describe("deleting a post cleans up its stored image", () => {
  let app;
  let StoredAsset;
  ...
  it("destroys the image and its ledger entry when its only post is deleted", async () => {
    ...
    await StoredAsset.create({ owner: alice.user.id, url: URL_A, publicId: ID_A });
    const postId = await postWithImage(alice.agent);

    expect((await alice.agent.delete(`/api/posts/${postId}`)).status).toBe(204);

    expect(destroy).toHaveBeenCalledWith(ID_A, { resource_type: "image", invalidate: true });
  ...
  it("keeps the image while another post still uses it", async () => {
    ...
    expect(await StoredAsset.countDocuments()).toBe(1);
  });
